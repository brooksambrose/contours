...
trn2sup.f <- function(reg2trn
                      ...
                      ,samp=F
                      ,aspect=1.5) {
  reg2trn <- copy(reg2trn) %>% {if(samp[1]==FALSE) . else if(length(samp)==1) .[.(sample(cmp, samp))] else .[.(samp),on='cmp']}
  mrows <- reg2trn[, .N, by = cmp][, max(N)]
  if(fntsz=='auto') fntsz <- min(500 / (min(mrows,28)), 12)*(1+aspect/5)
  pl <- suppressMessages(reg2trn[, .(pl = list({
    sd <- stringdist::stringdistmatrix(cr
                                       ,method = 'jw'
                                       ,p = thr
                                       ,useNames = 'strings')
    hc <- sd %>% hclust
    hc$height %<>% {scales::rescale_max(.,to = c(0,max_depth(hc %>% as.dendrogram)))*2}
    hd <- hc %>% as.dendrogram
    hd <- seriate_dendrogram(hd
                             , sd)
    hs <- get_nodes_attr(hd
                         , "height"
                         , simplify = T)
    l <- which(hs == 0)
    n <- which(hs != 0)
    ...
    p <- dendro_data(hd
                     , type = 'triangle') %>% {
                       ...
                       p + coord_flip(
                         clip = 'off'
                         ,xlim = c(n,1)
                         ,ylim =  c(-max(nchar(.$labels$label))-5, max(.$segments[, ec('y,yend')]))
                       ) + scale_y_continuous(position = 'right',expand = expansion(add=1)) +
                         ...
                         theme(axis.text.y = element_text(
                           ...
                           ,size = fntsz #/ ggplot2:::.pt
                           ,family = 'mono'
                         ),axis.text.x = element_blank(),aspect.ratio = aspect
                         )
                     }
    labels_colors(hd) <- c
  ...
  pc <- if ('time' %in% names(pd)) {
    ...
    1
  pd[, group := factor(group
                       , levels = rbc %>% sort)]
  pdup <- function(r
                   , input
                   , ix) {
    pd[data.table(cmp = ix
                  , r[, .(x
                          , y)])
       , on = ec('cmp,x,y')
       , `:=`(
         group = isolate(input$labeler)
         ,user = try(system('echo $USER'
                            , intern = T))
         %>% {
           if (inherits(.
                        , 'try-error'))
  ...
  pdpr <- function(ix) {
    withr::with_options(list(width = 1e4)
                        , prnablank(pd[.(ix)
                                       , on = 'cmp'
                                       , !ec('x,y,cmp')]))
    cat('\n\n')
  ...
  pldr <- function(ix) {
    ...
    pl[.(ix)
       ...
       , pl][[1]] +
      geom_point(
        data = get_nodes_xy(pl[.(ix)
                               , on = 'cmp'
                               , hd][[1]]
                            , type = 'triangle')[-which(get_nodes_attr(pl[.(ix)
                                                                          , on = 'cmp'
                                                                          , hd][[1]]
                                                                       , 'leaf'))
  ...
  onStop(function() {
    cat("\nSupervised training sets saved to:"
        , out)
  })
  runApp(
    shinyApp(
      ui = fluidPage(
        uiOutput("title")
        ,miniButtonBlock(actionButton('none'
                                      , 'None'
                                      , width = '60px')
                         ,actionButton('all'
                                       , 'All'
        ...
        ,div(style='text-align:center;margin: auto;max-height:500px;overflow-y:scroll;max-width:900px;overflow-x:scroll;'
             ,plotOutput(
               ...
               ,click = clickOpts("plot_click"
                                  , clip = T)
               ,dblclick = dblclickOpts('plot_dbl'
                                        , clip = F)
               ,height = 'auto'#'100%'
               #,width = '800px'#'100%'
               ,inline= F
      ...
      ,server = function(input
        ...
        observeEvent(input$plot_dbl
                     , {
                       updateRadioButtons(session
                                          , inputId = 'labeler'
                                          , selected = rbc[(which(isolate(input$labeler) == rbc) + 1) %>% {
                                            j <- .
                                            (length(rbc) + 1) %>% {
                                              (j %/% .) + (j %% .)
                                            }
                     ...
                     , priority = p)
        p %<>% `-`(1)
        ## 2 #input$pager #i$x # pager changes -> update global reactive index then redraw plot reprint table title
        i <<- reactiveValues()
        ...
        #         observeEvent(i$x
        #                      , {
        # if(pd[.(isolate(i$x)),on='cmp',.N==2]) hide('plot') else show('plot')                     }
        #                      , priority = p)
        ...
        observeEvent(i$x
                     , {
                       output$title <- renderUI({
                         tagList(miniTitleBar(sprintf('Component %s'
                                                      , pd[.(isolate(i$x))
                                                           , cmp[1]])))
                       })
                     }
        ...
        observeEvent(input$plot_click
                     , {
                       output$table <- renderPrint({
                         nd <- get_nodes_xy(pl[.(isolate(i$x))
                                               , on = 'cmp'
                                               , hd[[1]]]
        ...
        , {
          ...
          , height = 10+{pd[.(isolate(i$x)),on='cmp',.N>2]*(fntsz * aspect * pd[.(isolate(i$x)),.N])}
          , width= {
            pl[.(isolate(i$x)), on = 'cmp', hd[[1]] ] %>% dendextend::max_depth(.) * 2 +
              pd[.(isolate(i$x)),on='cmp',label %>% as.character %>% nchar %>% max] * fntsz
          })
        }
        , priority = p)
        p %<>% `-`(1)
