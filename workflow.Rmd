...
data.table::setDTthreads(threads = max(1,parallel::detectCores()-1)) # give data.table access to more cores
```
# Import WOK Data
...
if(file.exists(f)){
  ...
  dbl2bel<-plagiat::dbl2bel.f(wok2dbl,trim_doi = F,trim_anon = T,trim_loops = T,capitalize = T,trim_pendants = T)
  save(dbl2bel,file=f)
}
rm(f)
```
...
if(file.exists(f)){
  ...
  bel2mel<-plagiat::bel2mel.f(dbl2bel,type = 'crel',out = 'd/p')
  save(bel2mel,file=f)
}
rm(f)
```
