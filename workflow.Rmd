...
if(file.exists(f)){
  ...
  if(!'crm2pwc'%in%ls()) load('d/q/crm2pwc.RData')
  if(!'cg2sm'%in%ls()) load('d/p/cg2sm.RData')
}
  source('app.R')
  #unlink(f)
  trn2sup.f(reg2trn,cg2sm,fntsz='auto',aspect=1.5,out = f,samp=crm2pwc[ec('hld,trn'),on='mls',unique(cmp) %>% sort %>% {.[seq(1,length(.),by=100)[1:20]]}]) 
rm(f)
```
