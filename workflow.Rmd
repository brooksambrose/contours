...
'data.table,disk.frame,ggplot2,igraph,network,intergraph,forcats,shiny,shinyjs,miniUI,shinyPagerUI,dendextend,ggdendro' %>% ec %>% sapply(library,character.only = T) %>% invisible # load packages
...
if(!'smp'%in%ls()) {load('d/q/crm2pwc.RData');smp<-crm2pwc[ec('hld,trn'),on='mls',unique(cmp) %>% sort %>% {.[seq(1,length(.),by=100)[1:10]]}
];rm(crm2pwc)}
source('app.R')
options(shiny.reactlog = TRUE)
trn2sup.f(
  reg2trn,cg2sm,fntsz='auto',aspect=1.5,out = f,samp=smp
  ,dcache = T,cache = sub('\\.txt\\.gz','-cache',f) %>% {if(dir.exists(.)) . else {dir.create(.);.}}
  ,brwsr=rstudioapi::viewer,gather=2
)
rm(f)
```
...
```{r trn2sup-burndown}
d<-fread('d/qq/trn2sup.txt.gz')[,max(lubridate::ymd_hms(time),na.rm=T) %>%  data.table(first(.N)) %>% setnames(ec('l,n')),by=cmp][,ix:=.I][,t:=c(NA,diff(l))][,cmt:=c(NA,sapply(.I[-1],function(x) weighted.mean(t[2:x],n[2:x])))]
d[-1,lm(t~ix,weights=1/n) %>% {ggplot(aes(x=ix,y=t),data=.)+geom_smooth(data=.SD[-.N],aes(weight=1/n),color='red',linetype='dashed',method='lm',se = F,size=1)+geom_line(aes(y=cmt),color='green',size=1,linetype='dashed')+annotate(size=1,geom='segment',x=min(ix),xend=max(ix),y=min(.$fitted),yend=max(.$fitted),color='blue')+geom_point(aes(size=n,color=abs(.stdresid)))+theme_dark(base_family = 'serif')+theme(legend.position = 'none',panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),panel.grid.minor.y = element_blank())+scale_color_viridis_c(direction=1,option = 'inferno',limits=qnorm(p=c(.01,.99)))+ylab(NULL)+xlab(NULL)+scale_x_continuous(labels=cmp)}]
```
...
if(file.exists(f)){
  ...
  crm2fuz.f<-function(nsamp=1000,apprx=ec('ceiling,floor'),strict=F,plot=T,thr){
    ...
    trn2crm[,cr:=apply(.SD,1,function(x) x %>% unique %>% clp),.SDcols=!'ix']  # some original records did have redundant author and source, so will have to fix this later with accurate levels import
    ## try three different ways to regionalize ##
    ## sample to choose thresh
    tgt<-list(tgt=nsamp,apx=apprx[1])
    withr::with_seed(12345,sel<-trn2crm[,{
    ...
    trn2crm[,cra:={m<-character(.N);pbapply::pbsapply(1:.N,function(i) m[i]<<-setdiff(cr[-i],m[i]) %>% {.[stringdist::amatch(x = cr[i],table = .,method = 'jw',maxDist = thr,nomatch = NA_character_)]});m}]
    cel<-trn2crm[!is.na(cra),.(s=cr,r=cra,jwd=mapply(stringsim,a=cr,b=cra,method='jw',p=0))][!!(jwd-1),data.table(.SD %>% apply(1,sort) %>% t,jwd) %>% setnames(ec('s,r,jwd')),.SDcols=!'jwd'] %>% unique %>% setorder(-jwd)
