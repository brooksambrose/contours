...
setDTthreads(percent=25) # binder limits combined cpu load to 1 core's worth, so core use should be discounted
...
if(file.exists(f)){
  ...
  bel2mel<-list()
  sub('$','.txt',f) %>% {if(file.exists(.)) stop(sprintf('Remove %s before writing new lines.',.))}
  bel2mel$crel<-pbapply::pblapply(
    ...
    ,function(x) {
      r<-dbl2bel[x,.(ut=as.integer(ut),cr=as.integer(cr))][,data.table(do.call(rbind,lapply(1:(length(cr) - 1), function(y) matrix(cr[c(rep(y,length(cr) - y), (y + 1):length(cr))], ncol = 2)))),by = ut] %>% setnames(old=c('V1','V2'),new=c('cr1','cr2'))
      if(DF) fwrite(x = r,file = sub('$','.txt',f),append=T,quote = F,sep = '\t',nThread = 1) else r
    }
    ,cl=nc
  )
  sub('$','.txt',f) %>% {
    if(DF) {zip(zipfile = {z<-sub('$','.zip',.);z},files = .);file.remove(.)}
    dfbel2mel<-zip_to_disk.frame(zipfile = z,outdir = '.',nchunks=nc*4,shardby=ec('cr1,cr2'))[[1]]
