...
'cpumem.R' %>% {if(file.exists(.)) sprintf("Rscript -e \"source(\'%s\')\"",.)} %>% system(wait = F)
...
binder<-Sys.getenv('CPU_LIMIT') %>% as.numeric %>% is.na %>% `!`
''->hash
```
# Import WOK Data
...
```{r cpumem2}
'cpumem.R' %>% {if(file.exists(.)) sprintf("Rscript -e \"source(\'%s\')\"",.)} %>% system(wait = F)
if(binder) if(interactive()) browseURL(sprintf('https://hub.gke.mybinder.org/user/brooksambrose-contours-%s/rstudio/export/cpumem.png?name=cpumem.png&file=~cpumem.png',hash))
```
