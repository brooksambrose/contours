...
'vxtywstq'->hash
cpumem<-function(bnd=binder,hsh=hash) {
  # 'cpumem.R' %>% {if(file.exists(.)) sprintf("Rscript -e \"suppressPackageStartupMessages(source(\'%s\'))\"",.)} %>% system(wait = F)
  # Sys.sleep(2)
  # if(bnd) if(interactive()) {
  #   browseURL(sprintf('https://hub.gke.mybinder.org/user/brooksambrose-contours-%s/rstudio/export/cpumem.png?name=cpumem.png&file=~%%2Fcpumem.png',hsh))
  #   browseURL(sprintf('https://hub.gke.mybinder.org/user/brooksambrose-contours-%s/rstudio/files/cpumem.png',hsh))
  # }
}
cpumem(F)
```
